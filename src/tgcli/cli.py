...
from . import auth, chats, media, messages, resolve
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--json", action="store_true",
                        help="print raw tool output without line formatting")
    ...
    sp = sub.add_parser("login", parents=[common],
                        help="Log in (default: QR code)")
    sp.add_argument("--qr", action="store_true", default=False,
                    help="QR login (this is the default)")
    sp.add_argument("--phone", nargs="?", const="", default=None,
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print("tg: session is in use by another connection (AuthKeyDuplicatedError)", file=sys.stderr)
        print("hint: this CLI and the MCP daemon must use separate sessions; "
              "run `tg logout` then `tg login --qr` to get a fresh one", file=sys.stderr)
        return 2
        ...
        print("hint: check that the proxy is up (e.g. Clash 127.0.0.1:7890) "
              "and TELEGRAM_PROXY_* in .env", file=sys.stderr)
        return 2
